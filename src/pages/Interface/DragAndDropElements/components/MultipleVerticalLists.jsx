...
import { Card, CardHeader, CardTitle, Media, Avatar, AvatarAddOn } from '../../../../components';
              ...
              {(provided, draggableSnapshot) => (
                ...
                >
                  <Media>
                    ...
                    <Media body>
                      <span className="mt-0 h6 mb-1">{item.name}</span>
                      <p className="mb-0 text-muted text-truncate">{item.title}</p>
                    </Media>
                  </Media>
                </div>
              )}
  ...
  render() {
    ...
    return (
      <Draggable draggableId={id} index={index}>
        {(provided) => (
          <div className="col-md-4" ref={provided.innerRef} {...provided.draggableProps}>
...
export class MultipleVerticalLists extends React.Component {
  ...
  onDragEnd(result) {
    ...
    if (source.droppableId === 'board') {
      ...
      const lists = reorder(this.state.lists, source.index, destination.index);
            ...
            {(provided) => (
              <div className="row" ref={provided.innerRef} {...provided.droppableProps}>
                {_.map(lists, (list, index) => (
                  <Column id={list.id} index={index} title={list.title} key={list.id}>
                    <VerticalList listId={list.id} items={this.state[`${list.id}Items`]} />
                  </Column>
                ))}
              </div>
            )}
