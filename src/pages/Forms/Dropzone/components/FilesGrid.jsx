...
import { Col, Row, Card, CardBody, Button, UncontrolledTooltip } from '../../../../components';
        ...
        <Card className="mb-3">
          ...
          <CardBody className="pt-2">
            <div className="d-flex align-items-center mb-0 mt-0">
              <h6 className="text-truncate mb-0">{file.name}</h6>
              <Button
                color="link"
                onClick={() => {
                  onFileRemove(file);
            ...
            <div className="mb-0">
              by You &middot;{' '}
              <span className="text-uppercase">{`${numeral(file.size).format('0.00a')}B`}</span>
            </div>
            <div className="mb-0">{moment(file.modifiedDate).format('DD-MMM-YYYY, HH:mm')}</div>
          </CardBody>
        </Card>
      </Col>
    ))}
